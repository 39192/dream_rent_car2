...
function initializeTimeComboBox() {
  ...
  for (let c = 8; c <= 23; c++) {
    let u = c === 10 ? "selected" : "",
      l = (c < 10 ? "0" : "") + c + ":00";
    $("#return-time").append("<option value=" + l + " " + u + ">" + l + "</option>");
    if (c < 24) {
      let d = (c < 10 ? "0" : "") + c + ":30";
...
function processAboutAllChargesOf(e) {
  ...
  $("#" + e + "-basic-office").text(convertDollarText(basicCharge));
...
function getAdditionalTimeCharge(e) {
  ...
  // 1일 대여의 경우에만 추가 시간 요금 부과
  if (rentDays === 1) {
    if (additionalHours >= 0.5 && additionalHours < 1) {
      return 8;
    } else if (additionalHours >= 1 && additionalHours <= 1.5) {
      return 8;
    }
  }
